...
public class AVLTree implements Iterable<Integer> {
    ...
    protected Node root;
    protected Deque history = new ArrayDeque();
    ...
    public int updateSize(Node node){
        ...
        int size1 = (node.left != null) ? updateSize(node.left) : 0;
        int size2 = (node.right != null) ? updateSize(node.right) : 0;

        node.size = size1+ size2 +1 ;
        return node.size;
    }
    protected Node insertNode(Node node, int value) {
        ...
        if (node == null) {
            ...
            history.push(insertedNode);
            history.push(ImbalanceCases.NO_IMBALANCE);
            return insertedNode;
        }
        ...
        if (balance > 1) {
            if (value > node.left.value) {
                ...
                history.push(node.left);
                history.push(ImbalanceCases.LEFT_RIGHT);
            }
            node = rotateRight(node);
            history.push(node);
            history.push(ImbalanceCases.LEFT_LEFT);

        } else if (balance < -1) {
            if (value < node.right.value) {
                node.right = rotateRight(node.right);
                history.push(node.right);
                history.push(ImbalanceCases.RIGHT_LEFT);
            }
            node = rotateLeft(node);
            history.push(node);
            history.push(ImbalanceCases.RIGHT_RIGHT);
