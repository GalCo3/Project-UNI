...
public class BTree<T extends Comparable<T>> {
    ...
    protected Deque actions = new ArrayDeque<>();
    ...
    public void insert(T value) {
        ...
        if (root == null) {
            ...
            root.addKey(value);
            actions.push(value);
            actions.push(BTreeActionType.NONE);
        } else {
            ...
            while (currentNode != null && !wasAdded) {
                ...
                if (currentNode.getNumberOfKeys() == maxDegree - 1) {
                    T leftSplit = currentNode.keys[0];
                    T rightSplit = currentNode.keys[currentNode.numOfKeys-1];
                    actions.push(leftSplit);
                    actions.push(rightSplit);
                	actions.push(split(currentNode));
                    ...
                    hasChanged=true;
                    actions.push(BTreeActionType.SPLIT);
                }
    ...
    protected Node<T> getNode(T value) {
        ...
        while (node != null && !found) {
            int idx = node.getValuePosition(value);//2t-1
